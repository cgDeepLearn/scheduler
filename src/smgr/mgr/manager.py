...
class DemoManager(object):
    ...
    def create_task(cls, data):
        ...
        # 找到合适的worker来运行
        right_worker = demo_job_map.get_right_worker()
        if not right_worker:
        ...
        # map 操作对应信息
        demo_job_map.create_task_info(task_id, sub_jod_ids, right_worker)
        return 'ok'
    ...
    def delete_task(cls, task_id):
        """删除任务"""
        # 找到任务运行在哪个worker
        task_at_worker = demo_job_map.find_worker(task_id)
        if not task_at_worker:
            raise Exception("task not Existed")
        sub_jids = demo_job_map.get_task_info(task_id)["sub_jids"]
        req_data = {
        ...
        # 删除job_map对应信息
        demo_job_map.delete_task_info(task_id)
        return 'ok'
    ...
    def handle_register(cls, data):
        ...
        demo_job_map.set_worker(worker_host, sub_jids=all_sub_jids)
